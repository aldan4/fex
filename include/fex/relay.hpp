...
// The relay side of fex: the member registry (#3) and the commands that manage
// it, the object store and the roster it publishes (#6), capsule storage with
// its commit transaction (#6, #9), and the event loop that serves them (#4).
